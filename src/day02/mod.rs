...
pub(crate) fn part1(inp: &str) -> usize {
    parse_input(inp).enumerate().map(|(i, mut parts)| {
...
fn part2(inp: &str) -> usize {
    parse_input(inp).map(|parts| {
        let (red, green, blue) = parts.fold((0, 0, 0), |(red, green, blue), (num, color)| {
            match color {
...
fn parse_input(inp: &str) -> impl Iterator<Item = impl Iterator<Item = (usize, &str)>> {
    inp.lines()
        .map(|line| line.split_once(": ").unwrap().1.split("; ").flat_map(|p| p.split(", ")).map(|part| {
